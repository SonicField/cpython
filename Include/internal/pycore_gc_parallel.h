...
#include "pycore_gc.h"              // PyGC_Head, _gc_runtime_state
...
// Opt-in via --with-parallel-gc configure flag
// GIL builds only (free-threading uses different GC via pycore_gc_ft_parallel.h)
...
// =============================================================================
// Thread Handle Compatibility
// =============================================================================
//
// CPython 3.12 does not have PyThread_handle_t (added in 3.13+).
// We use platform-native thread handles directly for joinable threads.
//

#ifdef _POSIX_THREADS
#include <pthread.h>
typedef pthread_t _PyGC_ThreadHandle;
#elif defined(NT_THREADS)
#include <windows.h>
typedef HANDLE _PyGC_ThreadHandle;
#else
#  error "Parallel GC requires pthreads or Windows threads"
#endif
...
typedef struct {
    ...
    // Thread handle (platform-native, see _PyGC_ThreadHandle typedef above)
    _PyGC_ThreadHandle thread;
