...
class TestAdaptiveControllerBounds(unittest.TestCase):
    ...
    def test_random_walk_vs_fixed_workers(self):
        ...
        # Run with random walk (adaptive). Disable first in case setUp
        # left a different worker count enabled — enable_parallel(N) now
        # requires a matching count or a prior disable.
        try:
            gc.disable_parallel()
