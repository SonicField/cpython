...
class TestAdaptiveControllerConvergence(unittest.TestCase):
    ...
    # PRNG-flaky: the EMA hill-climb controller's convergence is sensitive to
    # the random seed and the realised distribution of explore/exploit steps.
    # The per-generation EMA controller is replaced by a biased constrained
    # random walk in a subsequent commit, at which point this test no longer
    # applies (the per-gen API it depends on is removed). Mark expectedFailure
    # so git bisect through this commit isn't tripped by a non-deterministic
    # assertion.
