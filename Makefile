...
MODBUILT_NAMES=      array  _asyncio  _bisect  _contextvars  _csv  _heapq  _json  _lsprof  _opcode  _pickle  _queue  _random  _struct  _xxsubinterpreters  _xxinterpchannels  _zoneinfo  audioop  math  cmath  _statistics  _datetime  _decimal  binascii  _bz2  _lzma  zlib  _dbm  _gdbm  readline  _md5  _sha1  _sha2  _sha3  _blake2  pyexpat  _elementtree  _codecs_cn  _codecs_hk  _codecs_iso2022  _codecs_jp  _codecs_kr  _codecs_tw  _multibytecodec  unicodedata  _crypt  fcntl  grp  mmap  ossaudiodev  _posixsubprocess  resource  select  _socket  spwd  syslog  termios  _posixshmem  _multiprocessing  _ctypes  _curses  _curses_panel  _sqlite3  _ssl  _hashlib  _uuid  _tkinter  xxsubtype  _xxtestfuzz  _testbuffer  _testinternalcapi  _testcapi  _testclinic  _testimportmultiple  _testmultiphase  _testsinglephase  _ctypes_test  xxlimited  xxlimited_35  atexit  faulthandler  posix  _signal  _tracemalloc  _codecs  _collections  errno  _io  itertools  _sre  _thread  time  _typing  _weakref  _abc  _functools  _locale  _operator  _stat  _symtable  pwd
...
abs_srcdir=	/data/users/alexturner/phoenix/cpython
abs_builddir=	/data/users/alexturner/phoenix/cpython
...
prefix=		/data/users/alexturner/phoenix/cpython-install
...
CONFIG_ARGS=	 '--prefix=/data/users/alexturner/phoenix/cpython-install' 'CC=clang' 'CXX=clang++'
...
# Phoenix JIT build via cmake
JIT_BUILD_DIR=Python/jit_build/build
JIT_LIBS= \
	$(JIT_BUILD_DIR)/libjit.a \
	$(JIT_BUILD_DIR)/libphoenix_jit.a \
	$(JIT_BUILD_DIR)/libcommon.a \
	$(JIT_BUILD_DIR)/libborrowed.a \
	$(JIT_BUILD_DIR)/libinterpreter.a \
	$(JIT_BUILD_DIR)/_deps/asmjit-build/libasmjit.a \
	$(JIT_BUILD_DIR)/_deps/fmt-build/libfmt.a

$(JIT_BUILD_DIR)/libjit.a: $(wildcard Python/jit/*.cpp Python/jit/**/*.cpp Python/jit_common/*.cpp)
	@mkdir -p $(JIT_BUILD_DIR)
	cd $(JIT_BUILD_DIR) && CC=clang CXX=clang++ cmake .. -DCMAKE_BUILD_TYPE=RelWithDebInfo -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ > /dev/null 2>&1
	cmake --build $(JIT_BUILD_DIR) --parallel $$(nproc)
	@echo "Phoenix JIT built successfully"

# Build the interpreter
$(BUILDPYTHON):	Programs/python.o $(LINK_PYTHON_DEPS) $(JIT_BUILD_DIR)/libjit.a
	$(CXX) $(PY_CORE_LDFLAGS) $(LINKFORSHARED) -o $@ Programs/python.o $(LINK_PYTHON_OBJS) -Wl,--start-group $(JIT_LIBS) -Wl,--end-group $(LIBS) $(MODLIBS) $(SYSLIBS) -lstdc++ -lz
...
Modules/_gdbm$(EXT_SUFFIX):  Modules/_gdbmmodule.o; $(BLDSHARED)  Modules/_gdbmmodule.o $(MODULE__GDBM_LDFLAGS)  -o Modules/_gdbm$(EXT_SUFFIX)
...
Modules/_tkinter$(EXT_SUFFIX):  Modules/_tkinter.o Modules/tkappinit.o; $(BLDSHARED)  Modules/_tkinter.o Modules/tkappinit.o $(MODULE__TKINTER_LDFLAGS)  -o Modules/_tkinter$(EXT_SUFFIX)
