...
// merged during GC.
//
// SHARDED MODE (Py_BRC_SHARDED):
// When enabled, each bucket has multiple shards indexed by the decrefing
// thread's ID. This reduces mutex contention when multiple threads decref
// objects owned by the same thread. Each thread state has one queue per
// shard, and a bitmap tracks which queues are non-empty.
#include "Python.h"
...
// ============================================================================
// SHARDED BRC IMPLEMENTATION
// ============================================================================

// Lock all shards in a bucket. Used for linked list modifications.
static void
lock_all_shards(struct _brc_bucket *bucket)
{
...
// Unlock all shards in a bucket (reverse order to avoid deadlock patterns).
static void
unlock_all_shards(struct _brc_bucket *bucket)
{
...
_Py_brc_queue_object(PyObject *ob)
{
    ...
    // Select shard based on THIS thread's ID (the decrefing thread)
    int shard_idx = (int)(my_tid % _Py_BRC_NUM_SHARDS);
    ...
    // Set bit in bitmap to indicate this shard has items.
    // Use atomic OR since consumer may read concurrently.
    _Py_atomic_or_uint16(&tstate->brc.non_empty_shards, (uint16_t)(1 << shard_idx));
...
_Py_brc_merge_refcounts(PyThreadState *tstate)
{
    ...
    // Read bitmap - relaxed load is fine, we double-check under lock
    uint16_t to_check = _Py_atomic_load_uint16(&brc->non_empty_shards);

    if (to_check == 0) {
        return;  // Fast path: nothing to merge
    }

    // Only lock shards that might have items
    for (int i = 0; i < _Py_BRC_NUM_SHARDS; i++) {
        if (!(to_check & (1 << i))) {
            continue;
        }
        ...
        // Merge this shard's queue into local stack
        _PyObjectStack_Merge(&brc->local_objects_to_merge, &brc->queues[i]);

        // Clear the bit - queue is now empty
        _Py_atomic_and_uint16(&brc->non_empty_shards, (uint16_t)~(1 << i));
...
_Py_brc_init_state(PyInterpreterState *interp)
{
    ...
    for (Py_ssize_t i = 0; i < _Py_BRC_NUM_BUCKETS; i++) {
        ...
        // Shard mutexes are zero-initialized by default
    }
}
...
_Py_brc_init_thread(PyThreadState *tstate)
{
    ...
    // Must lock all shards to safely modify the linked list
    lock_all_shards(bucket);

    brc->tid = tid;
    brc->non_empty_shards = 0;
    llist_insert_tail(&bucket->root, &brc->bucket_node);

    unlock_all_shards(bucket);
}
...
_Py_brc_remove_thread(PyThreadState *tstate)
{
    ...
    while (!empty) {
        ...
        // Lock all shards to check if all queues are empty
        lock_all_shards(bucket);
        ...
        if (empty) {
            // Clear bitmap and remove from linked list
            brc->non_empty_shards = 0;
...
{
    // Unlock all shard mutexes. Some may be locked because locks can be
    // handed off to a parked thread (see lock.c). We don't have to worry
    // about consistency here, because no thread can be actively modifying
    // a bucket, but it might be paused (not yet woken up) on a PyMutex_Lock
    // while holding that lock.
    for (Py_ssize_t i = 0; i < _Py_BRC_NUM_BUCKETS; i++) {
        for (int j = 0; j < _Py_BRC_NUM_SHARDS; j++) {
            _PyMutex_at_fork_reinit(&interp->brc.table[i].shards[j].mutex);
        }
    }
}
...
// ============================================================================
// ORIGINAL (NON-SHARDED) BRC IMPLEMENTATION
// ============================================================================
