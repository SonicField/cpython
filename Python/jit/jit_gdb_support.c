...
/* Simple string helper (already used throughout JIT) */
struct jit_string_t;
extern struct jit_string_t* ss_sprintf_alloc(const char* fmt, ...);
extern const char* ss_get_string(struct jit_string_t* s);
extern void ss_free(struct jit_string_t* s);
 ...
 */
__attribute__((weak, noinline)) void __jit_debug_register_code(void) {
  __asm("");
}
 ...
 * interface (e.g. LLVM ORC).
 */
__attribute__((weak)) JITDescriptor __jit_debug_descriptor = {
    1,
    JIT_NOACTION,
    NULL,
    NULL};
...
register_elf_ctx(ELFObjectContext* ctx, const char* type, void* ptr) {
  ...
  if (jit_get_config()->gdb.write_elf_objects) {
...
int register_raw_debug_symbol(
    ...
    size_t code_size,
    size_t stack_size) {
  if (!jit_get_config()->gdb.supported) {
    return 1;
  }
...
int register_pycode_debug_symbol(
  ...
  if (!register_elf_ctx(ctx, "PyFunctionObject", (void*)code)) {
    elfctx_free(ctx);
    return 0;
  }
 ...
 * implementation. There is much magic here... */
...
static ELFObjectContext* elfctx_new(
  ...
  ELFObjectContext* ctx =
      (ELFObjectContext*)calloc(1, sizeof(ELFObjectContext));
  if (ctx == NULL) {
    return NULL;
  }
