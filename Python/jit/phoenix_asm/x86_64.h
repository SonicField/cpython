 ...
 *
 * Closes W-C2 D-1777048937. Pre-DSecondary-2 (push 45 9e342abd7d) this
 * had to overload is_abs_addr=1 to get the SIB pure-disp32 encoding,
 * because PhxMem zero-init made base=RAX-by-default indistinguishable
 * from a real RAX base. DSecondary-2 added an explicit has_base flag
 * (mirror of has_index): constructors that take a base set has_base=1;
 * pure-displacement constructors leave it 0. encode_modrm_mem branches
 * on !has_base to emit `04 25 disp32` (SIB base=101). is_abs_addr is
 * now restored to its conventional 'absolute 64-bit address' semantics
 * and is unused here. */
static inline PhxMem phx_fs_ptr(int32_t offset) {
    ...
    /* has_base=0 (zero-init): no base register, encode as SIB pure-disp32 */
    return m;
}
