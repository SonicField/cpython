...
void* NativeGenerator::getVectorcallEntry() {
   ...
   * theologian 05:12:16Z diagnostic-purity flag: per-iter capacity read
   * preserved so H5 capacity-grows-mid-loop UAF is observable). */
  ...
  fprintf(stderr,
          ...
          phx_ptr_set_capacity(&refs), refs.count);
  for (size_t i = 0; i < phx_ptr_set_capacity(&refs); i++) {
    void *p = phx_ptr_set_at(&refs, i);
    if (p != nullptr) {
      fprintf(stderr,
              "X3B-XFER-ITEM: env=%p slot=%zu cap=%zu obj=%p rc=%ld\n",
              (void *)&func->env, i, phx_ptr_set_capacity(&refs),
              p, (long)Py_REFCNT((PyObject *)p));
      env_.code_rt->addReference(static_cast<PyObject *>(p));
      xfer_count++;
    }
  }
  fprintf(stderr, "X3B-XFER-DONE: env=%p xfer_count=%zu final_cap=%zu\n",
          (void *)&func->env, xfer_count, phx_ptr_set_capacity(&refs));
