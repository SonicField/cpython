...
#if PY_VERSION_HEX >= 0x030C0000
    int idx = jit_bc_instr_opcode_offset(bci);
    /* uninstrument only — do NOT unspecialize before the switch.
     * The switch returns recognized specializations as-is;
     * the default case unspecializes everything else. */
    int op = uninstrument(bci->code, idx);
    switch (op) {
...
int jit_bc_instr_is_absolute_control_flow(JitBytecodeInstr *bci) {
    return is_absolute_control_flow(bci);
}
