...
typedef struct {
    _PyGCFTParState *state;
    PyThreadState *tstate;   // Pre-created thread state (may be NULL)
...
parallel_worker_thread(void *arg)
{
    ...
    // Set thread-local pointers to the pre-created thread state.
    // We only set TLS pointers (no full _PyThreadState_Bind) because:
    // 1. Thread states are created before stop-the-world to avoid deadlock
    //    (_PyThreadState_New -> _Py_qsbr_reserve -> _PyEval_StopTheWorld)
    // 2. Full binding would require unbinding from this thread, but
    //    cleanup runs on the main thread after workers exit.
    // Only _Py_tss_tstate is needed — it satisfies reftotal_add() in
    // debug builds when tp_traverse calls Py_INCREF.
    PyThreadState *tstate = args->tstate;
    if (tstate != NULL) {
    ...
    // Unbind thread state (cleanup happens in _PyGC_TestParallelMark)
    if (tstate != NULL) {
        _Py_tss_tstate = NULL;
        _Py_tss_interp = NULL;
    }
                        ...
                        _PyGCFTParState *state)
{
    ...
    for (int i = 1; i < state->num_workers; i++) {
        ...
        thread_args[i - 1].interp = interp;
        thread_args[i - 1].tstate = (state->worker_tstates != NULL)
                                    ? state->worker_tstates[i - 1] : NULL;
...
_PyGC_TestParallelMark(int num_workers)
{
    ...
    // Pre-create Python thread states for worker threads BEFORE stopping
    // the world. _PyThreadState_New may call _Py_qsbr_reserve which can
    // call _PyEval_StopTheWorld internally, so it must not be called while
    // the world is already stopped (deadlock on stw->mutex).
    int num_tstates = num_workers - 1;  // worker 0 runs on main thread
    PyThreadState **worker_tstates = NULL;
    if (num_tstates > 0) {
        worker_tstates = PyMem_RawCalloc(num_tstates, sizeof(PyThreadState *));
        if (worker_tstates != NULL) {
            for (int i = 0; i < num_tstates; i++) {
                worker_tstates[i] = _PyThreadState_New(
                    interp, _PyThreadState_WHENCE_UNKNOWN);
                // NULL is OK — worker will run without reftotal tracking
            }
        }
    }
...
cleanup:
    ...
    // Clean up pre-created thread states (after world is restarted)
    if (worker_tstates != NULL) {
        for (int i = 0; i < num_tstates; i++) {
            if (worker_tstates[i] != NULL) {
                PyThreadState_Clear(worker_tstates[i]);
                PyThreadState_Delete(worker_tstates[i]);
            }
        }
        PyMem_RawFree(worker_tstates);
    }
...
scan_heap_pool_work(_PyGCThreadPool *pool, int worker_id)
{
    ...
    while (1) {
        int page_idx = _Py_atomic_add_int(&work->page_counter, 1);
