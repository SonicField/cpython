...
**Update 2026-04-24T04:39:18Z (post Tier 8 Phase A landing):** The 8-incident class was RESOLVED by root-cause attribution at b83f0840fb (push 31): the apparent 'external reverter' was our own `scripts/w45_section_3_5_derivation_drift.sh` restore_files() trap using `git checkout HEAD --` on TOUCHED_FILES, destroying pre-existing unstaged user modifications. Fix: snapshot-based restore preserving pre-script state. mtime-checkpoint discipline VINDICATED — caught self-inflicted infrastructure bug pre-commit (testkeeper 03:53:05Z). Lesson: 'external actor' hypotheses need a `grep -lE 'git checkout HEAD --' scripts/` first-step BEFORE detection-layer escalation. Per feedback_falsifier_restore_clobbers_unstaged.md (librarian 04:28:11Z institutional record).

**Rollback-audit 2026-04-24T04:41:28Z (per pythia #109 + supervisor):** The 4-step Pre-Edit WT Integrity discipline + mtime-checkpoint extension + expanded pre-commit gate were authored under FALSE-ACTOR framing (assumed external reverter). Root cause was internal infrastructure. Rollback decision: KEEP all three layers (each is independently validated discipline). Latency cost acknowledgment: expanded pre-commit gate adds ~17min per Tier 8 / multi-file builder*.{cpp,h,c} commit (compile ~80s + §3.5 BUILD MODE ~6min + per-bench gate ~10min). Cost justified for that commit-class; supervisor 00:39:20Z trivial-push exemption (script-only, doc-only) remains in force — DO NOT extend full gate to trivial pushes. mtime-checkpoint cost is per-write (~5s per touched file); acceptable for any multi-file edit. Future detection-layer additions: require explicit cost-benefit framing + cite the 8-incident false-actor lesson before authoring.

**Meta-rule reform 2026-04-24T05:59:04Z (per pythia #111 (4) + supervisor):** RETRO-FRAMING IS NOT SATISFACTION. The cost-benefit + 8-incident-cite requirement (above) MUST be applied AT AUTHORING of any new procedural rule, not retroactively. Retro-frames acknowledge the violation but do NOT satisfy the meta-rule. Two retro-frames have already happened (D-1777005990 self-violation + push-36 race-window citation rule); a third would calcify the failure mode into "gate becomes ritual that activates only when pythia surfaces the omission." Reform locks the meta-rule against ritualization: any agent authoring a new procedural rule must include cost-benefit framing + 8-incident-cite as part of the AUTHORING POST, OR the rule is held pending until that framing is added BEFORE adoption. Authors who notice they skipped the framing should self-flag and provide the framing in a follow-up post BEFORE the rule is treated as adopted.
